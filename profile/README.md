...
**A capability model and deferred-approval layer for [OpenClaw](https://github.com/openclaw/openclaw). Stock OpenClaw, no fork, installed as plugins.**
...
- **Writes are queued and simulated.** Every write goes through `submitAction()`, is journaled as pending, and is merged into the agent's subsequent reads as if it had happened. You apply, reject or revert it later — `clawos approvals apply 3`, or `/approvals apply 3` in a private operator chat.
...
|---|---|---|
| [`openclaw-os`](https://github.com/clawkeeper/openclaw-os) | Kernel plugin, shared contracts, gatekeeper kit, `clawos` CLI, installer, conformance suite, and the `fs` and `github` reference gatekeepers | Kernel: every line reviewed |
| [`gatekeepers`](https://github.com/clawkeeper/gatekeepers) | Community gatekeepers — one folder per service — plus the `write-gatekeeper` skill any agent can follow to build one | Normal |

## What's proven, what isn't

| | |
|---|---|
| Kernel, `fs` driver, cells, installer, config reconcile, backups | **Accepted** — Phase 3: 78/78 conformance, 98/98 kernel-live checks, 23 real model turns on `openclaw@2026.9.2`; 410/410 host tests. Paste a `file://` URL → grant → `gk_fs_dir_list` → audit → `clawos grant revoke` removes the tool next turn; non-operators can't introduce. |
...
| Approvals TUI, blueprints, `clawos update`/rollback, MCP/HTTP drivers | **Not built.** Designed in `docs/implementation-plan.md`; placeholders only. |
...
clawos status --json
clawos grant add --agent <agent> file:///home/you/projects/some-folder
